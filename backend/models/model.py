...
# Dedicated classifier to determine if an image is a skin image.
...
try:
    ...
    class DummyClassifier:
        def predict(self, image_array):
            # Always returns a probability of 1 (skin image)
            return np.array([[1.0]])
    skin_classifier = DummyClassifier()

# Load YOLOv5s model using ultralytics (auto-downloads if needed)
...
def detect_face_yolo(image_path, confidence_threshold=0.5):
    ...
    Returns True if any detection with class 'person' is found.
    """
...
def is_skin_image_classifier(image_path):
    """
    Uses a dedicated binary classifier to check if the image is a skin image.
...
def predict(image_path):
    # Step 1: YOLO-based detection to reject images containing people.
    if detect_face_yolo(image_path):
        return "Invalid", "Please post a skin image (face/object detected)."
    
    # Step 2: Use the dedicated classifier to check if it is a skin image.
    if not is_skin_image_classifier(image_path):
        return "Invalid", "Please post a skin image (non-skin content detected)."
    
    # Step 3: Process the image with the lesion detection model.
    image_array = preprocess_image(image_path)
